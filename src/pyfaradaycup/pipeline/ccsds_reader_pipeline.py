...
import re, sys, struct, copy, os, datetime
...
def wrapper_status(path='', verbose=False, gzip=False, spconly=False):
	...
	data = {'wrap_met':[], 'wrap_apid':[], 'data_met':[], 'data_apid':[], 'wrap_seq':[], 'data_seq':[]}
	...
	pkt_inds = np.array([(m.start(0), m.end(0)) for m in re.finditer(pattern, bytestr, re.DOTALL)])
	try:
...
def read_file(path='', verbose=False, gzip=False):
	...
	#Loop through each packet beginning and decommutate it
	for i_pointer, pointer in enumerate(pkt_starts):
		foo = read_bytestr(bytestr, pointer+12, data, apidformat, pktcnt, verbose=verbose)
	
		#Update status
		nowtime = time.time()
		if (nowtime-updatetime) >0.5:
			sys.stdout.write('\b'*40 + '{:5.1f}% Complete.  ET={:6.2f} sec.'.format( ( np.double(i_pointer) ) / npackets * 100.  , nowtime-starttime))
			updatetime = nowtime
	
	#write out a summary of how things went
	nowtime = time.time()
	sys.stdout.write('\b'*40 + '{:5.1f}% Complete.  ET={:6.2f} sec.\n\n'.format( 100., nowtime-starttime))						
	sys.stdout.write('Packet Summary\n')
	for thisapid in pktcnt[0].keys():
		sys.stdout.write('\tAPID {:}: found {:7.0f} packets\n'.format(hex(thisapid), pktcnt[0][thisapid] ) )
	sys.stdout.write('\n')
...
def read_file_sc(path='', verbose=False, ptp=False, gzip=False):
	...
	with open('/psp/data/sc_hsk/L1/APID257_combined.txt') as f: lines = f.readlines()
	...
	sc_hk_filenames = 	{'05.01.01':'SPP.SC.HK.05.01.01_G01.blk',
	...
	try:
		...
		if ( (cchead['CCSDS_Version']!=0) | 
			 (cchead['CCSDS_PacketType']!=0) | 
			 (cchead['CCSDS_SecHdrFlag']!=1) ):
			raise ValueError('CCSDS header values not as expected')
		file_dt = datetime.datetime(2010,1,1)+datetime.timedelta(seconds=cchead['CCSDS_MET'])
		try:
		...
		try:
			match = re.search(os.path.sep+'20[1-5][0-9]'+os.path.sep+'[0-9][0-9][0-9]'+os.path.sep,path).span()
			file_dt = datetime.datetime(int(path[match[0]+1:match[0]+5]),1,1) + datetime.timedelta(days = int(path[match[0]+6:match[0]+9])-1)		
			try:
				good_time = np.where(vers_dt<file_dt)[0][-1]
			except IndexError:
				good_time = 0
			...
			print('***WARNING*** Could not find date based on filename...using most recent')
	...
	apidformat = {}
	for apid in ok_apids:
		apidformat[apid], lengths[apid] = get_layout_sc(apid, verbose=verbose, filename=os.path.join('sc_hk_def',sc_hk_filename))
		if apidformat[apid]:
			data[apid]={}
			for name in apidformat[apid].names:
				data[apid][name]=[]
	...
	for key,val in lengths.items(): lengths[key] = 2*np.ceil((val)/2.).astype(int) - 7
	...
	if ptp:
		pattern = struct.pack('3B',0x03,0x00,0xBB) #2,3,4,5,6th bytes (start from zero) of PTP header
		pattern += b'.'*12
		pattern += b'('
		for inst_ap in ok_apids: #allowable SPC instrument apids
			pattern += struct.pack('2B',(2048+inst_ap & 0xff00)>>8, 2048+inst_ap & 0x00ff)	
			pattern += b'|'
		...
		for inst_ap in ok_apids: #allowable SPC instrument apids
			...
			if inst_ap==0x256:
				...
				pattern += struct.pack('2B',(lengths[inst_ap] & 0xff00)>>8, lengths[inst_ap] & 0x00ff)	
			pattern += b'|'
		pattern = pattern[:-1] #get rid of that last "|"
		pattern += b')'
		
		offset_bytes = 0 #we searched for beginning of CCSDS packets, so no offset necessary
	#import pdb; pdb.set_trace()
	...
	#Loop through each packet beginning and decommutate it
	for i_pointer, pointer in enumerate(pkt_starts):
		foo = read_bytestr(bytestr, pointer+offset_bytes, data, apidformat, pktcnt, verbose=verbose)
...
def read_bytestr(bytestr, pointer, data, apidformat, pktcnt, verbose=False):
	...
	try:
		...
		if verbose: print('Full CCSDS Header Not Present')
		return()
	...
	if ccsds_head['CCSDS_Version']!=0:
		if verbose: print('CCSDS Version is invalid')
		return()
	
	if ccsds_head['CCSDS_PacketType']!=0:
		if verbose: print('CCSDS Type is invalid')
		return()
	
	if ccsds_head['CCSDS_SecHdrFlag']!=1:
		if verbose: print('CCSDS Secondary Header flag is invalid')
		return()
	
	#Make sure the full packet is here
	if pointer+pkt_len+7 > len(bytestr):
		if verbose: print('Full CCSDS packet not available at end of bytestr')
		return()
	...
	if apid in apidformat.keys():
		...
		parse_pkt(thispkt, data, apidformat, apid, ccsds_head) #could send this off to a parallel task?  Might try that if too slow this way					
		
	else:
		if apid in pktcnt[1].keys():
			pktcnt[1][apid]+=1
		else:
			pktcnt[1][apid]=1
...
def parse_ccsds_head(bytestr, verbose=False):
	...
	head['CCSDS_MET'] = 2**24*bytearr[6] + 2**16*bytearr[7] + 2**8*bytearr[8] + bytearr[9]
...
def parse_pkt(bytestr, data, apidformat, apid, ccsds_head, verbose=False):
	...
	if len(bytearr)*8. < sum(form.bits): 
		print('short packet: {:}'.format(hex(apid)))
		return
	...
	if hasattr(form, 'sw_data_vars'):
		...
		newdat = {}
		for key in form.sw_data_vars: newdat[key]=[]
...
def get_layout(apid, verbose=False):
	try:
		...
		if verbose: print("***INFO*** No local 'sweap_tlm.blk' found...using the one near ccsds_reader_pipeline.py")
		try:
	...
	for i, line in enumerate(lines):
		...
		if line[0:8]=='APID_{:}'.format(hex(apid)[2:].zfill(3)).upper():			
			if verbose: print('APID {:} Format Found'.format(hex(apid)[2:]).upper())
			thisapid = apid_obj()
			thisapid.apid=apid
			...
			while line[0:4]!='APID':
				...
				try:
					...
					elif ((line.strip()[0:9]=='( SW_DATA') | (line.strip()[0:12]=='( SW_SPC_SCI')):
			...
			start = np.array([0] + [sum(thisapid.bits[0:i]) for i in range(1,len(thisapid.bits))])
			length = np.array(thisapid.bits)-1
			thisapid.bytestart = np.floor(start/8.).astype(int)
			thisapid.bitstart = start-8*thisapid.bytestart.astype(int)
			endbits = start+length
			thisapid.byteend = np.floor(endbits/8.).astype(int)
			thisapid.bitend = endbits-8*thisapid.byteend.astype(int)

			return(thisapid)
			
	#if we didn't find that APID
	print('***ERROR*** [ccsds_reader_pipeline] Did not find APID {:}'.format(hex(apid)[2:]).upper())
	return(None)
...
def get_layout_sc(apid, verbose=False, filename=''):
	try:
		file = open(filename)
		print('using sc_hk file: {:}'.format(filename))
	except:
	...
	lines = file.readlines()
	for i, line in enumerate(lines):
		if line[0:11]=='SC_HK_0x{:}'.format(hex(apid)[2:].zfill(3).upper()):			
			if verbose: print('APID {:} Format Found'.format(hex(apid)[2:]).upper())
			thisapid = apid_obj()
			thisapid.apid=apid
...
if __name__=="__main__": read_file(ptp=False, verbose=True)
